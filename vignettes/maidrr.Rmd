...
Call `insights(mfit, vars, data, interactions = 'auto', hcut = 0.5, pred_fun = NULL)` with arguments:
...
* `interactions`: string specifying how to deal with interaction effects (only two-way interactions allowed).
    + `"none"`: interaction effects between features are not included in the output.
...
* `data`: data frame containing the original training data.
* `lambda`: numeric value or numeric vector for the penalty parameter lambda (see formula below).
    + numeric value: same penalty parameter lambda is used for all features in `fx_vars`.
    + numeric vector: feature-specific lambdas, must satisfy `length(lambda) == length(fx_vars)`.
...
gr_data_fr <- fx_vars_fr %>% segmentation(data = mtpl_fr,
                                          lambda = ifelse(grepl('_', names(fx_vars_fr)),
                                                          0.00001,
                                                          0.0001))
gr_data_fr %>% 
  dplyr::select(dplyr::ends_with('_')) %>% 
