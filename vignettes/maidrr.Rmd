...
1. `insights`: obtain insights from a black box model in the form of feature effects.
1. `segmentation`: perform segmentation of observations based on the grouping of feature effects.
1. `surrogate`: fit a surrogate generalized linear model (GLM) to the segmented data.
1. `explain`: explain the prediction of a surrogate GLM via each feature's contribution.
...
Call `insights(mfit, vars, data, interactions = 'user', hcut = 0.5, pred_fun = NULL)` with arguments:
...
Call `segmentation(fx_vars, data, lambda)` with arguments:
...
* `type`: string specifying the type of segmentation to perform. There are two options:
    ...
    + `"lambdas"`: the optimal number of groups is determined by the penalized loss (see formula below).
* `values`: numeric value or named numeric vector with the value(s) for `ngroups` of `lambdas`.
    + numeric value: same value is used for all features in `fx_vars`.
    + named numeric vector: feature-specific values, must satisfy `length(values) == length(fx_vars)` and `names(values)` must be the same as the comment attributesof the effects in `fx_vars` (`unlist(lapply(fx_vars, comment))`).
...
The first part measures how well the effect *y* is approximated by the grouped variant *g* in the form of a mean squared error over all levels/values of the feature. The second part measures the complexity of the grouping by means of the common logarithm of the number of groups for the feature. The penalty parameter $\lambda$ acts as a bias-variance trade-off. A low value will allow a lot of groups, resulting in an accurate approximation of the effect. A high value will enforce using fewer groups, resulting in a coarse approximation of the effect. For a specified value of $\lambda$, the optimal number of groups *k* follows by minimizing the penalized loss function. The grouping in `maidrr` is done via the `Ckmeans.1d.dp` package.

We segment the BE portfolio with the same penalty parameter for all features and interactions. The grouped features are added to the `data` with a trailing underscore in their column name. The number of groups says someting about the importance of each feature, but notice the difference in number of groups between the main and interaction effects. Due to the scale difference in `y` between the main and interaction effects (see comment earlier + penalized loss function formula), it is best to use distinct values of $\lambda$ for main and interaction effects.
```{r segmentation_be}
...
We segment the FR portfolio by different $\lambda$ values, one for the main effects and a smaller one for the interaction:
```{r segmentation_fr}
gr_data_fr <- fx_vars_fr %>% segmentation(data = mtpl_fr,
                                          type = 'lambdas',
                                          values = setNames(ifelse(grepl('_', names(fx_vars_fr)),
                                                                   0.00001, 0.0001),
                                                            unlist(lapply(fx_vars_fr, comment))))
gr_data_fr %>% 
  dplyr::select(dplyr::ends_with('_')) %>% 
  dplyr::summarise_all(dplyr::n_distinct)
```
...
* `group_pd`: groups the effect by making use of `rpart` or `Ckmeans.1d.dp` depending on the feature type.
...
Let's explain the prediction made by the BE GLM for two policyholders (id 34 and 4938). Notice that the prediction for the former is below the intercept of the GLM, while it is more than doubled for the latter. Why is this the case? The low risk for id 34 is mainly thanks to a low bonus-malus level and high age. Driving a high powered car increases the risk on the other hand. The high risk for id 4938 is mainly due to a high bonus-malus level and young age. Driving a low powered car decreases the risk on the other hand. Note that these contributions and standard errors or shown on the response scale after taking the invere link function of the coefficients. For a Poisson GLM with `log`-link function this implies applying the `exp` function on the coefficients, with contributions on the multiplicative response scale and a value of 1 indicating no effect of that feature.
...
These are of course just toy examples with random segmentation choices, namely the value(s) of $\lambda$, to illustrate the functionalty of `maidrr`. Tuning the value of $\lambda$ is very important to obtain competitive prediction models. Therefore we incorporate a function to automate this task for you: `maidrr::autotune`.

## Automated lambda tuning
<a id="auto"></a>
The `maidrr` workflow from black box to surrogate via `bb_fit %>% insights(...) %>% segmentation(...) %>% surrogate(...)` is highly dependend on the value(s) of $\lambda$ supplied to `maidrr::segmentation`. The grouping and selection of features is entirely dependend on $\lambda$. Any ad-hoc choice is most likely going to result in a surrogate which is not really competitive with the original black box model, so it is important to choose the value(s) for $\lambda$ in an optimal way. To automate this tuning task, `maidrr` contains two functions: `autotune(...)` and `autofit(...)`. The former iterates over a grid of $\lambda$'s and calculates cross-validation errors for each grid value, while the latter uses the tuning results to fit the optimal surrogate model. Let's get into some more detail on both functions.


### Tuning via cross-validation
Call `autotune(fx_vars, data, target, lambdas = as.vector(outer(seq(1, 10, 0.1), 10^(-7:3))),``nfolds = 5, strat_vars = NULL, glm_par = alist(), err_fun = mse, ncores = -1)` with arguments:

* `fx_vars`: list of data frames containing the feature effects, preferably the output of `maidrr::insights`.
* `data`: data frame containing the original training data.
* `target`: string specifying the name of the target (or response) variable to model.
* `lambdas`: numeric vector with the possible $\lambda$ values to explore. The search grid is created automatically via `maidrr::lambda_grid`. This grid contains only those values of $\lambda$ that result in a unique grouping of the full set of features. If both main and interaction effects are present in `fx_vars`, a seperate grid is generated and all the pairwise combinations are evaluated in a cartesian grid search.
...
* `err_fun`: function to calculate the prediction errors on the validation folds. This must be an R function which outputs a single number and takes two vectors `y_pred` and `y_true` as input for the predicted and true target values respectively. An additional input vector `w_case` is allowed to use case weights in the error function. The weights are determined automatically based on the `weights` field supplied to `glm_par`. The `maidrr` package contains the following functions as examples:
...
```{r autotune_be, eval = FALSE}
set.seed(5678)
tune_be <- fx_vars_be %>% autotune(data = mtpl_be,
                                   target = 'nclaims',
...
```{r lambda_grid}
lmbd_grid <- maidrr::lambda_grid(fx_vars = fx_vars_be, lambda_range = as.vector(outer(seq(1, 10, 0.1), 10^(-7:3))))
lmbd_grid
length(unique(lmbd_grid$lambda_main))
length(unique(lmbd_grid$lambda_intr))
```

```{r autotune_be2}
lmbd_grid %>% dplyr::slice(1) %>% 
           unlist(., use.names=TRUE)
```


### Fitting the optimal surrogate
Call `autofit(autotune_out, fx_vars, data, target, glm_par = alist())` with arguments:

* `autotune_out`: data frame with the results from `maidrr::autotune`.
* `fx_vars`: list of data frames containing the feature effects, preferably the output of `maidrr::insights`.
* `data`: data frame containing the original training data.
* `target`: string specifying the name of the target (or response) variable to model.
* `glm_par`: named list, constructed via `alist()`, with additional arguments to be passed on to `glm()` (see the section on `surrogate`). Note however that the argument `formula` will be ignored as the formula is determined automatically by the `target` and the selected features during the tuning process.
